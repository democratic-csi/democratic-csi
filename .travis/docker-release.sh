...
if [[ -n "${TRAVIS_TAG}" ]]; then
  docker buildx build --progress plain --pull --push --platform "${DOCKER_BUILD_PLATFORM}" -t ${DOCKER_REPO}:${TRAVIS_TAG} .
  ...
  if [[ "${TRAVIS_BRANCH}" == "master" ]]; then
    ...
    docker buildx build --progress plain --pull --push --platform "${DOCKER_BUILD_PLATFORM}" -t ${DOCKER_REPO}:${TRAVIS_BRANCH} .
